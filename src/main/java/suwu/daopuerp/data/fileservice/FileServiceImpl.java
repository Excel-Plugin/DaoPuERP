...
import suwu.daopuerp.util.AnnotationUtil;
import suwu.daopuerp.util.PathUtil;
import suwu.daopuerp.util.ReflectionUtil;
    ...
    public T saveTuple(T entity) {
        ...
        for (int i = 0; i < columns.size(); i++) {
            if (fields[i].getAnnotation(JsonSerialize.class) != null) {
                try {
                    if (AnnotationUtil.isKeyAutoGenerated(clazz)) {
                        ...
                        //如果是update那么使用参数id，如果不是，那么创建新id
                        String serId = (String) json.get(id);
                        fields[i].setAccessible(true);
                ...
                if (jsonSerialize != null) {
                    try {
                        if (AnnotationUtil.isKeyAutoGenerated(clazz)) {
                            ...
                            String serId = "";
                            if (idName != null && idName.length() > 0) {
                                serId = (String) jsonObject.get(idName);
                            }
                            fields[i].setAccessible(true);
